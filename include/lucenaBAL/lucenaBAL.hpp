...
/**
  ...
  - FIXME: something that is a known bug, incomplete, or a likely trouble area
    targeted for a future rewrite. These tags typically decorate in-development
    code, but may persist across multiple releases depending on triage
    outcomes.

  - APIME: a likely API problem. These tags make note of surprising
    behavior that may pose a future maintenance problem, and are typically the
    result of poor design in the library or its dependencies.

  - OPTME: something that could be made more efficient with a high probable
  ...
  - SEEME: something noteworthy that requires care to use correctly. These tags
    demarcate a counterintuitive flow or algorithmic approach that can be
    tricky to understand or that involves non-obvious tradeoffs.
