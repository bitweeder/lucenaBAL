...
*/
  ...
  //
  //	SEEME - bitweeder
  //	Apple LLVM 9.1 is the first release that actually advertises C++17
  //	support (as per __cplusplus); previously, it only had C++14 with almost
  ...
  //	SEEME - bitweeder
  //	Technically, we should be able to function perfectly well with older
  //	versions of Apple LLVM, but Apple deprecates anything more than about 3
  //	years old, so it would be somewhat masochistic to fight it and
  //	undertake all that testing ourselves. Note that iterations of the
  //	current lucenaBAL design were previously tested with versions as far
  //	back as  LLVM 9.4+, and the largely-compatible older design was tested
  //	with much older versions going all the way back to the Apple gcc days.
  //	However, support for CFM, Carbon, etc, all got dropped long ago, so
  //	this is academic.
  #endif
  ...
  //
  //	FIXME - bitweeder
  //	Separate support should be added for c2/clang.
  ...
  //
  //	SEEME - bitweeder
  //	This must appear after clang, as the llvm derivatives have a tendency to
  //	define gcc’s macros; Apple LLVM, in particular, likes to pretend to be a
  //	crufty, ancient version of gcc.
  ...
  //
  //	SEEME - bitweeder
  //	This must always appear last, as other compilers have a tendency to
  //	emulate MSVC by defining its macros.
  //
  //	SEEME - bitweeder
  //	Our version requirement is arguably conservative, but supporting earlier
  //	versions would require significantly more work.
  #if (_MSC_FULL_VER < 190'024'210)
