...
/*------------------------------------------------------------------------------
  ...
  SEEME - bitweeder
  Re: SD-6 macros: full support started getting rolled out as of MSVC 2019
  16.0, but testing with that series didn’t start until 16.3. As a result, it’s
  unclear exactly when the various macros started going live. For that reason,
  prior to 16.3, we fall back to compiler and language version checks, but
  after that, we rely on the SD-6 macros. It’s possible some supported features
  will fall through the cracks with some versions, but it’s unlikely; all the
  same, this will be updated as new information becomes available.
*/
...
//	`_CPPUNWIND` is not an exact analog, but seems to serve.
  ...
  //	since our earliest supported dialect is C++11.
      ...
      //	SEEME - bitweeder
      //	Available in `_MSC_VER``1900` for partial, unsure of full.
...
//	C++17 features
      ...
      //	SEEME - bitweeder
      //	This is implemented, but it’s unclear if/when the DR that affected
      //	the proposal was addressed.
...
#else
  #error                                                                       \
      "lbalMSVCPostInitialization.hpp was directly included while using the wrong compiler"
#endif
