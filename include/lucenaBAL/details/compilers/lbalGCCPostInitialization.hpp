...
//	previews.
    ...
    #if !defined(LBAL_CPP14_AGGREGATE_NSDMI) && (LBAL_GCC_VERSION >= 50000)
      #define LBAL_CPP14_AGGREGATE_NSDMI 201304L
    #endif

    #if !defined(LBAL_CPP14_BINARY_LITERALS) && (LBAL_GCC_VERSION >= 40900)
    ...
    #if !defined(LBAL_CPP14_DECLTYPE_AUTO) && (LBAL_GCC_VERSION >= 40900)
      #define LBAL_CPP14_DECLTYPE_AUTO 201304L
    #endif

    #if !defined(LBAL_CPP14_GENERIC_LAMBDAS) && (LBAL_GCC_VERSION >= 40900)
      #define LBAL_CPP14_GENERIC_LAMBDAS 201304L
    #endif

    #if !defined(LBAL_CPP14_INIT_CAPTURES) && (LBAL_GCC_VERSION >= 40900)
  ...
  //	SEEME - bitweeder
  //	`__cpp_sized_deallocation`—and therefor
  //	`LBAL_CPP14_SIZED_DEALLOCATION`—can be conditionally disabled, and we
  //	have no way of telling without the SD-6 macro. For this reason, we
  //	don’t provide a version override.

    #if !defined(LBAL_CPP14_VARIABLE_TEMPLATES) && (LBAL_GCC_VERSION >= 50000)
...
//	previews.
    ...
    #if !defined(LBAL_CPP17_CAPTURE_STAR_THIS) && (LBAL_GCC_VERSION >= 70000)
      #define LBAL_CPP17_CAPTURE_STAR_THIS 201603L
    #endif

    #if !defined(LBAL_CPP17_CONSTEXPR_LAMBDA) && (LBAL_GCC_VERSION >= 70000)
      #define LBAL_CPP17_CONSTEXPR_LAMBDA 201603L
    #endif

    #if !defined(LBAL_CPP17_DEDUCTION_GUIDES) && (LBAL_GCC_VERSION >= 70000)
      #define LBAL_CPP17_DEDUCTION_GUIDES 201606L
    #endif

    #if !defined(LBAL_CPP17_DEDUCTION_GUIDES_NB) && (LBAL_GCC_VERSION >= 80000)
      #define LBAL_CPP17_DEDUCTION_GUIDES_NB 201611L
    #endif

    //	SEEME - bitweeder
    //	The minimimum version is speculative; I couldn’t find a reference to
    //	indicate when - or if - gcc addressed this DR, aside from vague
    //	references to it being part of C++17. Note that gcc 8 was the first
    //	version claiming full C++17 language support, while gcc 9 has
    //	most--or-all of the C++17 library support; DRs are a different story,
    //	however.
    #if !defined(LBAL_CPP17_DEDUCTION_GUIDES_DR) && (LBAL_GCC_VERSION >= 80000)
    ...
    #if !defined(LBAL_CPP17_FOLD_EXPRESSIONS) && (LBAL_GCC_VERSION >= 60000)
    ...
    #if !defined(LBAL_CPP17_INLINE_VARIABLES) && (LBAL_GCC_VERSION >= 70000)
      #define LBAL_CPP17_INLINE_VARIABLES 201606L
    #endif

    #if !defined(LBAL_CPP17_NAMESPACE_ATTRIBUTES) && (LBAL_GCC_VERSION >= 60000)
    ...
    #if !defined(LBAL_CPP17_STRUCTURED_BINDINGS) && (LBAL_GCC_VERSION >= 70000)
  ...
  //	SEEME - bitweeder
  //	None of these have SD-6 macros, so we rely on a compiler version check
  //	and a language test.
    ...
    //	check and a language test. This will miss back-ported extensions, but
    //	will pick up preview implementations.
      ...
      //	SEEME - bitweeder
      //	This did not have a proper SD-6 token value until Dec 2019.
    ...
    //	FIXME - bitweeder
    //	gcc hasn’t yet shipped a fully functioning version of this feature.
    //	In particular, the latest one from gcc 8 fails when it encounters
    //	`#__VA_OPT__`. We treat this as unavailable, but it’s actually a good
    //	candidate for a new “partial implementation” value, or a special
    //	token (`LBAL_OOPS_xxx`?).
...
//	previews.
  ...
  #if LBAL_cpp_version > LBAL_CPP11_VERSION
    #if !defined(LBAL_CPP14_ATTRIBUTE_DEPRECATED) && (LBAL_GCC_VERSION >= 40900)
...
#else
  #error                                                                       \
      "lbalGCCPostInitialization.hpp was directly included while using the wrong compiler"
#endif
