...
*/
  ...
  //	Identify vector instruction sets, if any.
      ...
      //	SEEME - bitweeder
      //	There are no independent tests for these in MSVC, and it’s unclear if
      //	support for these particular instruction sets is implied for 64-bit
      //	architectures, as is the case for SSE1 and SSE2; we make an
      //	assumption, as these should be available in any post-2011 Intel-class
      //	CPU.
  ...
  //	SEEME - bitweeder
  //	There appear to be no intrinsics for detecting AVX-512 support in MSVC,
  //	yet, though support may be undocumented or under-reported. It’s
  //	currently moot, though, as we don’t support this instruction set family
  //	at this time.
  #endif

  //	SEEME - bitweeder
  //	These are unverified for Win32
...
//	SEEME - bitweeder
//	We assume the availability of 64-bit ints under supported versions of
//	MSVC, and also assume the unavaibility of intrinsic 128-bit ints.
  ...
  //
  //	SEEME - bitweeder
  //	This violates the rule about defining preprocessor macros with
  //	double-underscores (among other things). The usage should be safe.
  ...
  //	SEEME - bitweeder
  //	The key missing component for C99 compliance was correct variadic macro
  //	handling. This is in place as of MSVS 2019 16.0, and may have been
  //	available sooner with the use of the `/permissive-` switch. Note that as
  //	of at least MSVS 2019 16.3, the preprocessor is actually almost
  //	completely compliant with the C11 standard.
  //
  //	SEEME - bitweeder
  //	It’s unclear if there is a method for controlling preprocessor compliance.
  #if (_MSC_VER >= 1920)
  ...
  //
  //	SEEME - bitweeder
  //	In the absence of `__cpp_threadsafe_static_init` - i.e., in MSVS 2017 or
  //	earlier - there isn’t a way to determine whether this has been disabled
  //	(which can be done with `/Zc:threadSafeInit-`). We’ll just assume it
  //	_has_ been disabled, which is not ideal, but unavoidable without
  //	incorporating a test app into the build process.
  ...
  //
  //	SEEME - bitweeder
  //	In the absence of `__cpp_sized_deallocation` - i.e., in MSVS 2017 or
  //	earlier - there isn’t a way to determine whether this has been disabled,
  //	which can be done with `/Zc:sizedDealloc-`. We’ll just assume it _has_
  //	been disabled, which is not ideal, but unavoidable without incorporating
  //	a test app into the build process.
  ...
  //	SEEME - bitweeder
  //	These appear to be required by DLLs, as they like to save fallback
  //	versions of inline functions as object code in case the header version is
  //	unavailable for some reason; if not for this requirement, these would
  //	just be LBAL_VIS_HIDDEN.
...
#else
  #error                                                                       \
      "lbalMSVCInitialization.hpp was directly included while using the wrong compiler"
#endif
