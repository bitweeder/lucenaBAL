...
*/
  ...
  //	SEEME - bitweeder
  //	Apparently, `__STDC_VERSION__` only gets set if we’re using the C
  //	compiler, so this test doesn’t work. Since we’re requiring C++11 or
  ...
  //	SEEME - bitweeder
  //	There is no `LBAL_cpp_version` check since this can be conditionally
  //	disabled. We explicitly disable this if we don’t get a hit on the SD-6
  //	macro, as otherwise there’s no way to detect it—in particular, there is
  //	no `__has_extension()` value that can detect it.
  ...
  //
  //	SEEME - bitweeder
  //	This explicit test is here for documentation, but could actually be
  //	dropped as the common test will serve just as well.
  ...
  //
  //	SEEME - bitweeder
  //	Apple continues to lag vanilla clang by over a year in odd ways. As of
  //	Apple LLVM 10.0, their clang compiler advertises full C++17 compliance,
  //	but a number of feature test macros fail (and then there’s the real
  //	Standard Library deficiencies...):
  //
  ...
  //	SEEME - bitweeder
  //	<https://en.cppreference.com/w/cpp/compiler_support> reports that this is
  //	non-compliant, but that’s not exactly correct. Rather, Apple selectively
  //	disables detection for features it doesn’t want to advertise support for,
  //	typically because they would break ABI and/or they’ve had the
  //	implementation stripped out. Additionally, Apple lags a bit in updating
  //	the token values to reflect updates; it’s not always clear if this is an
  //	oversight or if an update has been held back.
...
//	SEEME - bitweeder
//	The “inline” declaration is extraneous, but required due to a currently
//	unaddressed clang defect (cf., the elusive PR32114).
...
//	SEEME - bitweeder
//	The first definition is a fallback if it turns out I’ve misunderstood the
//	requirements on general inline functions.
//	#define LBAL_VIS_INLINE_FUNC LBAL_VIS_INLINE_TEMPLATE_MEMBER_FUNC
  ...
  //	SEEME - bitweeder
  //	Using the internal_linkage attribute is equivalent to using the static
  //	keyword in C, and is semantically preferrable to inlining. However, what
  //	we -really- want is be able to strip this down to just LBAL_VIS_HIDDEN,
  //	but this requires linker support we don’t have, yet. See
  //	<http://lists.llvm.org/pipermail/cfe-dev/2018-July/058450.html> and
...
#else
  #error                                                                       \
      "lbalAppleClangInitialization.hpp was directly included while using the wrong compiler"
#endif
