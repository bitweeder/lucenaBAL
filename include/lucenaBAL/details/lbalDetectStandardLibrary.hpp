...
*/
  ...
  //	SEEME - bitweeder
  //	We use `__apple_build_version__` as a proxy for detecting Apple’s
  //	hacked-up version of libc++; I’m unaware of more reliable way to
  //	determine this. Thankfully, Apple doesn’t bastardize `_LIBCPP_VERSION`
  //	the same way it does with `__clang__`, etc.
  ...
  //	SEEME - bitweeder
  //	Note that older iterations of libstdc++ used __GLIBCPP__.
  #define LBAL_NAME_STANDARD_LIBRARY                                           \
  ...
  //	SEEME - bitweeder
  //	Not exactly an equivalent test, but I don’t know of a way to reliably
  //	identify the MSVC Standard Library: we simply check to see if we’re
  //	running MSVC and and whether no other Standard Library detection test has
  //	passed. Necessarily, this test must be performed last among the Library
  //	detection tests.
  #define LBAL_NAME_STANDARD_LIBRARY                                           \
    u8"MSVC Standard Library version " LBAL_Stringify_(_MSC_VER)

  #define LBAL_TARGET_STANDARD_LIBRARY_MSVC 1
#else
  //	APIME If we can’t determine the Standard Library implementation
