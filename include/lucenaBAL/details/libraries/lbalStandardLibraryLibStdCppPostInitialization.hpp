...
/*------------------------------------------------------------------------------
  ...
  SEEME - bitweeder
  One source of aggravation in libstdc++ is that there is no simple way to
  figure out what version of the library you are using. `__GLIBCXX__` is a
  release date, which can be interleaved across versions of the library
  depending on whether there are minor releases and bug fixes. We are forced to
  use `__GNUC__` as a proxy, which is not totally unreasonable, as long as the
  user hasn’t done something pathological like using an old libstdc++
...
*/
  ...
  //	C++17 Library Features
      ...
      //	SEEME Technically, this is only conditionally available.
      #if !defined(LBAL_LIBCPP17_IS_AGGREGATE)
        #define LBAL_LIBCPP17_IS_AGGREGATE 201703L
      #endif  //	LBAL_LIBCPP17_IS_AGGREGATE

      //	SEEME This trait spent a portion of its life with the original name
      //	`is_callable`; gcc had this straightened out by 7.1
        ...
        #define LBAL_LIBCPP17_CONSTEXPR_STRING 201611L
      #endif  //	LBAL_LIBCPP17_CONSTEXPR_STRING

      #if !defined(LBAL_LIBCPP17_CONSTEXPR_STRING_VIEW)
        #define LBAL_LIBCPP17_CONSTEXPR_STRING_VIEW 201611L
  ...
  //	SEEME Any C++17 library feature that requires gcc 9+ is detected
  //	correctly by `<version>`, so we don’t bother with redundant
  //	testing.
  #endif  //	LBAL_CPP17_VERSION

  //	C++20 Library Features
  //	SEEME Support for most C++20 library features is detected correctly by
  //	`<version>`, so we only track features that were _not_ detected
...
#else
  #error                                                                       \
      "lbalStandardLibraryLibStdCppPostInitialization.hpp was directly included with the incorrect Standard Library implementation"
#endif
