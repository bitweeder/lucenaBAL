...
*/
  ...
  /*
    ...
    SEEME - bitweeder
    Apple LLVM inherited a token `<version>` from libc++ 7; the first
    meaningful iteration appeared in libc++ 8, which ships— somewhat
    intact—with Xcode 11. That iteration was out of sync with the SD-6 doc,
    however, and has gotten even more out-of-sync. One of the upshots is that
    we can’t fully trust the `<version>` bundled with any shipping Xcode, e.g.,
    some of the token names have drifted from the official ones, and some of
    the token values have changed (and not just been updated).
  */
...
#else
  #error                                                                       \
      "lbalStandardLibraryAppleLibCppInitialization.hpp was directly included with the incorrect Standard Library implementation"
